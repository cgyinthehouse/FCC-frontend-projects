...
const ThemeToggle:React.FC = () => {
  ...
  return (
    <button
      ...
      onClick={toggle}
    >
      <span className={`${dark ? "" : "-translate-y-[22px]"}  left-[4px] top-[4px] absolute transition`}>🌙</span>
      <span className={`${dark ? 'translate-y-[16px]' : '-translate-y-[6px]'}  left-[4px] top-[4px] absolute transition`}>🔆️</span>
    </button>
  );
};
